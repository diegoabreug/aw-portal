...
def new_client():
    if request.method == 'POST':
        # Convert HTML checkbox string to a boolean integer for SQLite
        has_partner = 1 if request.form.get('has_partner') else 0

        conn = get_db()
        cursor = conn.cursor()

        # Insert main client data into the relational table
        cursor.execute('''
            INSERT INTO clients (
        ...
        ''', (
            ...
            # Cast financial inputs to float, defaulting to 0 to prevent DB errors
        ...
        # Extract dynamic arrays generated by the frontend JavaScript
        ...
        # Iterate through the submitted arrays to build the 1-to-many relationship
        for i in range(len(account_types)):
            # Only insert if the account type field is not empty
            if account_types[i]:
                cursor.execute('''
                ...
                ''', (
                    ...
                    # Safely handle potential missing values in the last4 array
...
def edit_client(client_id):
    ...
    if request.method == 'POST':
        ...
        # Replace old accounts with new ones.
...
def download_reports(client_id, report_id):
    ...
    # We use a generic name, since it could be any bank.
